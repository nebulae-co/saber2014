...
```{r Colmaps_deptos}
plot_depto <- function(depto_name){
  depto_data <- RESULTADO %>% filter(depto == depto_name)
  depto_map <- colmaps::map_departamentos(depto_name)

  colmaps::colmap(map = depto_map, data = depto_data, var = "grupo", map_id = "id", data_id = "id", autocomplete = TRUE) + ggtitle(depto_name)
  
}

deptos_graphs <- lapply(X = departamentos@data[["depto"]], plot_depto)

departamentos@data[["depto"]] -> names(deptos_graphs)
