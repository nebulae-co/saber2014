...
ecdf_municipios <- nuevo_saber_11 %$%
  tapply(X = Total, INDEX = id_Municipio, FUN = ecdf)
...
numero_grupos<-10
nombres_grupos<-LETTERS[c(4, 5, 6, 9, 8, 7, 3, 1, 2, 10)]
...
factor(nombres_grupos[grupos_municipios], ordered = TRUE) -> grupos_municipios
...
RESULTADO <- data.frame(
  id = sprintf("%05d", municipios_50$id_Municipio),
...
dff_to_plot %>%
  ggplot +
  aes(x = variable, y = id_municipio, fill = value, color = grupo) +
  geom_raster() +
...
```{r Colmaps}
library("colmaps")
colmap(map = municipios, data = RESULTADO, var = "grupo")

```


```{r Kruskal-Wallis, eval=FALSE, echo=FALSE, include=FALSE}
