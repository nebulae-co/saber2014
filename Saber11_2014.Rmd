...
fun_distance <- function(a, b, ...){
  ...
  diff_2_y <- sapply(X = 1:(n-1),
                     FUN = function(x, v, a, b){
                       (mean(v[x] >= a) - mean(v[x] >= b))^2 
                     },
                     v = valores, a = a, b = b,
                     USE.NAMES = FALSE)
  
  drop(diff_x %*% diff_2_y)
}

# Partir los totales por municipio

totales_por_municipio <- nuevo_saber_11 %$% split(x = TOTAL, f = MUNI_RESIDE)

# Calcular la matriz de distancias

cluster_municipios <- municipios_50$MUNI_RESIDE %>%
  expand.grid(., ., stringsAsFactors = FALSE) %>%
  apply(MARGIN = 1, 
        FUN = function(x){
          fun_distance(totales_por_municipio[[x[1]]],
                       totales_por_municipio[[x[2]]])}) %>% 
  matrix(nrow = nrow(municipios_50), ncol = nrow(municipios_50),
         dimnames = list(municipios_50$MUNI_RESIDE, municipios_50$MUNI_RESIDE)) %>% 
  as.dist %>% # Convertirlo a distancia
